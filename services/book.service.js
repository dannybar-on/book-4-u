    ...
    switch (filterBy.price) {
      ...
      case '50':
        return gBooks.filter(
          (book) => book.listPrice.amount < 50 && book.listPrice.amount > 20
        );
      case '100':
        return gBooks.filter(
          (book) => book.listPrice.amount < 100 && book.listPrice.amount > 50
        );
      case '101':
...
function addBook(book) {
  ...
  console.log(book);
  const newBook = _createBookFromApi(book);
  books.unshift(newBook);
  _saveBooksToStorage(books);
  return Promise.resolve();
}
function _createBookFromApi(book) {
  const { id } = book;
  const { title, subtitle, authors, description, publishedDate, pageCount } = book.volumeInfo;
...
function _createBooks() {
  if (!gBooks) gBooks = storageService.loadFromStorage(KEY);
